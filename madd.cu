...
__global__ void kernel_madd(float* A, float* B, float* C, int M, int N, float coef1, float coef2);
...
void cu_madd(float* A, float* B, float* C, int M, int N, float coef1, float coef2)
{
	float* d_a, * d_b, * d_c;
	...
	kernel_madd << < grid, blk >> > (d_a, d_b, d_c, M, N, coef1, coef2);
